...
load_path = os.path.join(os.getcwd(),'Trained_model_V','actor_td3')


# Because I saved the parameters instead of the model
# We need to initiate an actor network to load the model

class Network_II(nn.Module):
    def __init__(self, lr, input_dims, n_fc1, n_fc2,n_fc3,n_fc4,n_actions, name):
        super(Network_II, self).__init__()
...
def choose_action(observation,act_net,noise,time_step,warmup=500):
    if time_step < warmup:
...
#   just the normal gym process
    ...
    env = gym.make(env_id)
    act_net = Network_II(lr=3e-4,input_dims=input_dims,n_fc1=256,n_fc2=256,n_fc3=256,n_fc4=256,n_actions=8,name='actor')
